...
func _handle_pick_target(dst: Territory) -> void:
        ...
        if to_send < MIN_SENT_UNITS:
                ...
                _source_id = -1
                return
        print("Атакуем цель id=", dst.territory_id, " из источника id=", _source_id, ", отправляем=", to_send)
...
func _resolve_battle(src: Territory, dst: Territory, attackers: int) -> void:
        ...
        print("Итог боя: units источника=", src.get_units(), ", units цели=", dst.get_units())
